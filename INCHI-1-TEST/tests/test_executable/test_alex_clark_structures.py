...
def sdf_path():
    return (
        Path(__file__).parent.absolute().joinpath("data/alex_clark_structures.sdf.gz")
    )
...
@pytest.mark.xfail(
    ...
    reason="Molecular inorganic functionality is not used;",
)
def test_alex_clark_structures(sdf_path, run_inchi_exe):
    exectuable_results = [
...
def test_alex_clark_structures_statistics(sdf_path, run_inchi_exe):
    error_ids = []
    inchis = {}
    inchi_ids = []
    error_mols = []
